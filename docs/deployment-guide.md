...
React Native 앱에서 일기 텍스트를 보내면, LangChain + Claude를 통해 감정 분석 결과를 반환한다.

## 기술 스택

- Python 3.11+ / FastAPI / Uvicorn
- LangChain + langchain-anthropic (ChatAnthropic)
- Pydantic v2
...
[felling-api.sedoli.cloud] (HTTPS, 443)
    ↓ Synology 역방향 프록시
...
├── config.py              # 환경변수 로드, ChatAnthropic 설정
...
- **API 도메인**: https://felling-api.sedoli.cloud
...
#### Credential 등록 (Claude API Key)
1. **Manage Jenkins** → **Credentials** → **(global)** → **Add Credentials**
2. Kind: **Secret text**
3. Secret: Claude API 키
4. ID: `claude-api-key`
...
| 소스 프로토콜 | HTTPS |
| 소스 호스트 | `felling-api.sedoli.cloud` |
| 소스 포트 | 443 |
| 대상 프로토콜 | HTTP |
| 대상 호스트 | `localhost` |
| 대상 포트 | `8100` |
...
`felling-api.sedoli.cloud` → NAS 외부 IP (A 레코드 또는 CNAME)

### 5. SSL 인증서

**제어판** → **보안** → **인증서**에서 `felling-api.sedoli.cloud` 도메인용 Let's Encrypt 인증서 발급

### 6. React Native 앱 연동

앱 설정 화면에서 서버 URL을 `https://felling-api.sedoli.cloud`로 변경
