...
- `config.py` — two LLM instances: `llm` (512 tok, /analyze) and `llm_summary` (2048 tok, month + weekly)
...
- **Entry sampling**: month summary truncates at `MAX_ENTRIES=1000` (uniform-step sample) and `MAX_CONTENT_CHARS=400` per entry. Weekly caps at `WEEKLY_MAX_ENTRIES=60`.
